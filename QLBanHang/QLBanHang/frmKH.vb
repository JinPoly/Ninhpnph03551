Public Class frmKH
    ...
    Private Function InsertKH()
        ...
        sqlQuery += String.Format("VALUES('{0}','{1}','{2}','{3}')", txtMaKH.Text, txtTen.Text, txtDiaChi.Text, txtSDT.Text)
    ...
    Private Function UpdateKH()
        Dim sqlQuery As String = String.Format("UPDATE dbo.KhachHang SET TenKH ='{0}', DiaChi = '{1}', SDT = '{2}' WHERE MaKH = '{3}'", _
                                    Me.txtTen.Text, Me.txtDiaChi.Text, Me.txtSDT.Text, Me.txtMaKH.Text)
        Return _DBAccess.ExecuteNoneQuery(sqlQuery)
    End Function

    'dinh nghia ham kiem tra gia tri truoc khi insert vao CSDL
    Private Function Trong() As Boolean
        Return String.IsNullOrEmpty(txtMaKH.Text) OrElse String.IsNullOrEmpty(txtTen.Text) OrElse String.IsNullOrEmpty(txtDiaChi.Text) OrElse String.IsNullOrEmpty(txtSDT.Text)
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        Else
            If _Edit Then
                If UpdateKH() Then
                    MessageBox.Show("Sửa dữ liệu thành công!", "(-_-)", MessageBoxButtons.OK)
            ...
            Else
                If InsertKH() Then
                    MessageBox.Show("Thêm dữ liệu thành công!", "(-_-)", MessageBoxButtons.OK)
                    Me.DialogResult = Windows.Forms.DialogResult.Yes
                Else
